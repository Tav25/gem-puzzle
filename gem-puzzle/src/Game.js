...
const gameX = {
  ...
  initZeroPoint() {
    const zeroPositionCell = (this.zeroPosition + 1);
    cl(`cell:${zeroPositionCell}`);
    if (zeroPositionCell - 4 > 0) { cl(`y0 ${zeroPositionCell - 4}`); }
    if (zeroPositionCell % 4 != 0) { cl(`y1 ${zeroPositionCell + 1}`); }
    if (zeroPositionCell + 4 < 16) { cl(`y2 ${zeroPositionCell + 4}`); }
    if (zeroPositionCell % 4 != 1) { cl(`y3 ${zeroPositionCell - 1}`); }
  },
